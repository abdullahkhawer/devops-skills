...
1. NEVER run commands that delete, destroy, or modify production resources. Read only commands are allowed, and all mutating actions must be performed by a human.
2. NEVER deploy code, infrastructure, or configuration changes — a human must do this
3. NEVER run `terraform apply`, `terraform destroy`, `kubectl apply`, `kubectl delete` against production
4. NEVER read or output secrets, API keys, tokens, or credentials from files or environment variables
5. NEVER run commands with `sudo`
6. NEVER run `rm -rf` on any path outside the current project directory
7. NEVER disable, bypass, or modify the guardrail hooks in `.claude/hooks/`
8. NEVER push to `main` or `master` branches directly
